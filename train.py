...
def main():
    ...
    args.outer_loop, args.inner_loop = 1,1#get_loops(args.ipc)
    ...
    wandb.init(project="dds", name=args.method + '_' + args.dataset + '_' + args.model + '_' + str(args.ipc) + '_' + args.eval_mode + '_' + args.dsa_strategy + '_' + args.dis_metric)
    ...
    for exp in range(args.num_exp):
        ...
        if not os.path.exists('images_all.dat'):
            #memory mapped array
            images_all = np.memmap('images_all.dat', dtype='float32', mode='w+', shape=(len(dst_train), channel, im_size[0], im_size[1]))
            ...
            with open('indices_class.pkl', 'wb') as f:
                pickle.dump(indices_class, f)

        images_all = np.memmap('images_all.dat', dtype='float32', mode='r', shape=(len(dst_train), channel, im_size[0], im_size[1]))
        with open('indices_class.pkl', 'rb') as f:
